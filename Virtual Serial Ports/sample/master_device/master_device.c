...
static long master_ioctl(struct file *file, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	switch(ioctl_num){
		...
		case master_IOCTL_MMAP:
			copy_from_user(&data_size, (size_t*)ioctl_param, sizeof(data_size));
			while(offset + BUF_SIZE < data_size){
				ksend(sockfd_cli, page_address + offset, BUF_SIZE, 0);
				offset += BUF_SIZE;
			}
			ksend(sockfd_cli, page_address + offset, data_size - offset, 0);
			ret = 0;
			break;
